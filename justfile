...
dev-shell *args:
    #!/usr/bin/env bash
    set -euo pipefail
    docker build -t rmcp-server-builder-devcontainer .devcontainer/
    ...
    if [[ $# -eq 0 ]]; then
        exec docker run "${run_args[@]}" rmcp-server-builder-devcontainer bash
    else
        exec docker run "${run_args[@]}" rmcp-server-builder-devcontainer "$@"
    fi
